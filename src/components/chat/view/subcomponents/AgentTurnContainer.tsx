...
export default function AgentTurnContainer({
  ...
  if (turn.isActivelyStreaming) {
    const allMessages = turn.finalMessage
      ? [...turn.intermediateMessages, turn.finalMessage]
      : turn.intermediateMessages;
    // Re-insert finalMessage at its logical position (end)
    return (
      <div className="space-y-3 sm:space-y-4">
        {allMessages.map((msg, i) =>
          renderMessage(msg, i, i > 0 ? allMessages[i - 1] : null)
        )}
      </div>
    );
  }
        ...
        <details className="group">
          ...
          <div className="mt-2 ml-2 pl-3 border-l-2 border-gray-200 dark:border-gray-700 space-y-3 sm:space-y-4">
            {turn.intermediateMessages.map((msg, i) =>
              renderMessage(msg, i, i > 0 ? turn.intermediateMessages[i - 1] : null)
            )}
          </div>
        </details>
      )}
      {turn.finalMessage && renderMessage(turn.finalMessage, 0, null)}
    </div>
  );
}
